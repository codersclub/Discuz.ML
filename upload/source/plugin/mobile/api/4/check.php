...
if(in_array('mobile', $_G['setting']['plugins']['available'])) {
	...
	if (!$_G['cache']['wsq_checkinfo'] || TIMESTAMP - $_G['cache']['wsq_checkinfo']['expiration'] > 600) {
		...
		loadcache('userstats');
		// Since the micro-community relies on discuzversion == X3.2, and the code of the micro-community platform cannot be modified
		// So in X3.5 use truediscuzversion to mark the correct Discuz! Version.
		$array = array(
			'discuzversion' => 'X3.2',
