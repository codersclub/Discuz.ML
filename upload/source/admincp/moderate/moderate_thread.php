...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	if($moderation['validate']) {
		...
		$firsttime_validatethread = array();//Post approved for the first time
		$uids = array();				
		...
		if($firsttime_validatethread) {//Passed the first review, release news
			require_once libfile('function/post');
			require_once libfile('function/feed');
			$forumsinfo = C::t('forum_forum')->fetch_all_info_by_fids($forums);//Requires "allowfeed" information, allows to push the feed, and pushes broadcasts by default
			$users = array();
			foreach ($uids as $uid) {
				$space = array('uid'=>$uid);
				space_merge($space, 'field_home');//Requires ['privacy']['feed']['newthread'] info
				$users[$uid] = $space;
			}
