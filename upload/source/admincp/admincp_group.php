...
<?php
		...
		foreach ($groups as $id => $gforum) {
			...
			if(!empty($forums[$id])) {
				foreach ($forums[$id] as $forum) {
					...
					$lastfid = 0;
					if(!empty($subs[$forum['fid']])) { // The group is no longer displayed (obsolete code)
	...
	if($ajax && $_GET['formhash'] == formhash()) {
		...
		if(!$confirmed) {
			...
			$threads = C::t('forum_thread')->count_by_fid($fid); // The group is no longer displayed (obsolete code)
			$formhash = formhash();
