...
//		$this->var['setting']['jspath'] = $this->var['siteroot'] . 'static/js/';
	...
	private function _init_cnf() {// This new method is used to preload the configuration file, which is convenient to control the initialization process by using the options in the configuration file through $this->config when initializing the environment
	...
	private function _init_config() {// The original method of setting the site based on the configuration file retains the original method name, and instead uses $this->var['config'] to read and write config
		...
		// DISCUZ_DEPRECATED is used to identify functions or methods that are about to be discarded in the Discuz! X system, which is convenient for the maintenance of second-site sites or the development of third-party plug-ins
		// A system error after enabling means that the relevant functions will be deleted or throw exceptions in subsequent versions. We recommend that you do not continue to rely on functions that have thrown exceptions under this flag
	...
	private function _xss_check() {
		...
		if(isset($_GET['formhash']) && $_GET['formhash'] !== formhash()) {
			if(defined('CURMODULE') && constant('CURMODULE') == 'logging' && isset($_GET['action']) && $_GET['action'] == 'logout') {
				header("HTTP/1.1 302 Found");// Fix the bug of "Your current access request contains illegal characters and has been rejected by the system" occasionally when you click to exit multiple times
